...
- **Part C:** done (this commit). `extractJobFileText` writes `files/{fileId}/content/text` after upload (embedded PDF text / text/plain; images `none`; Word/Excel `unsupported`; 80_000 cap). No OCR. No OpenAI. No `src/` changes. Functions-only PDF lib: `unpdf@1.8.1`. Initial JS gzip **268.3 KB**. Staging: `functions:extractJobFileText` + `firestore:rules` after this commit. Production untouched. No backfill.
...
**Next:** Phase 13 Part C is done (this commit). Do not start Part D or Phase 14 until named. Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in.
