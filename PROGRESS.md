...
Latest branch **`phase-14-ask`**. Restore tag **`pre-phase14-2026-09-07`** (`0dfcb51`, last Phase 13 blocker). Phase 14 **Parts A and B are done** (`askRisingAmp` router; palette runs `src/queries/` and paints existing rows). Part C is next. Production is **untouched** (original six functions; no `askRisingAmp`). Localhost stays on staging. Never `--force`. The model never calculates. Gzip ceiling **400 KB** (current **270.1 KB**).
...
Phase 13 is **done on `phase-13-query-layer`**, plus a **Part D amendment**. Production is **untouched**. Phase 14 Parts A and B are **done** on `phase-14-ask`; Part C is next. Restore tag `pre-phase14-2026-09-07` (before Phase 14). Older: `pre-phase13-2026-09-06`.
...
- **Phase 14:** open on `phase-14-ask`. Restore tag `pre-phase14-2026-09-07` (`0dfcb51`). **Parts A and B done.** Router callable `askRisingAmp` (`retry: false`; `gpt-4o-mini`; ADR `docs/adr-ask-model.md`) returns a route only. The palette calls it, runs `src/queries/` with membership scope, and paints existing rows from `formatCents`. Production untouched (no `askRisingAmp`). No hosting. Part C is next.
...
**Not done, and next:** Phase 14 Part C. Parts A and B are done. 400 KB remains the held ceiling. The build still fails on breach. Optional: phone force-close twice for the Phase 12 worker; Overview vs History on a known job.
...
- [ ] Phase 14 Part C — working line, uncoded pool, honest refusal
