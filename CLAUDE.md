...
5. **Next:** Phase 9 **Part C** — upload, compress, thumbnails. One part per session. Do not deploy unless he names it. Leftovers still parked: App Check **enforcement**, ledger rollups, normalising stored money fields, TanStack Query on the ledger, dismantling the remaining AppContext ledger blob, `PHASE6-INTEGRITY.md`.
...
Brief: `PHASE9.md`. Mockup: `design/risingamp-files-vision.html`. Branch: `phase-9-job-files`. Restore tag: `pre-phase9-2026-08-31`. **One part per session.** Parts A and B are done. Next is Part C: upload, compress, thumbnails. No folders. No video. Do not deploy unless named.
