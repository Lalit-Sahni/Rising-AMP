...
1. Confirm git branch. Latest work is **`phase-15-actions`**. Restore tag: `pre-phase15-2026-09-08` (before Phase 15 code; SHA `176002f` on `phase-14-ask`). Phase 15 **Parts A–E are done** (action layer; dropped invoice → expense; sort uncoded to cost plan; activity view with undo; evals and kill switch). Do **not** start the next phase until this branch is merged / the owner names it. Phase 14 Parts A–G are done on `phase-14-ask`. Phase 15 **lifts** Phase 14’s “no writes” rule for reversible internal actions only — see `PHASE15.md`. The model still never calculates and never chooses the tier. GST is the stated figure only (never divide by 11). Phase 13 Parts A1–E plus blockers are **done on `phase-13-query-layer`**. Production is **untouched** (still the original six functions; no `askRisingAmp` on production). Phase 12 is **closed** and **live on production hosting** (6 Sep 2026; record `PHASE12.md`). **Phase 11 Parts A–E are live on production** (hosting, Firestore rules, `maintainLedgerRollup`, `ledgerRollup/current`) as of 5 Sep 2026, restore tag `pre-phase11-2026-09-05`. Never commit to `master` or `main`. Older restore tags: `pre-phase14-2026-09-07`, `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
2. Read `PROGRESS.md` (next concrete step), then `PHASE15.md`, then `PHASE14.md`, then `PHASE13.md`. Open `design/risingamp-ask-vision.html` before writing Ask code; `design/risingamp-vision.html` before changing the shell. `PHASE10.md` is the closed Cost Plan record. `PHASE9.md` through `PLAN.md` are closed records. `ARCHITECTURE.md` is how the running app is built. `DATABASE.md` is the living database guide.
...
5. **Next:** Phase 15 is **done on `phase-15-actions`**. Do **not** merge, do **not** deploy, do **not** start the next phase until the owner names it. Parts A–E are **done**. Phase 13 blockers closed on staging: uncoded pool; Lalit + Sydney Excavation merges; re-extract. Metro Consulting and the cross-kind unlinked list remain the owner’s. Production is **untouched**. Localhost stays on staging. The model never calculates. GST is the stated figure only. Production functions stay `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile` and `maintainLedgerRollup` — **no `askRisingAmp` on production**. Staging also has `extractJobFileText` and `askRisingAmp` (`retry: false`). Staging Firestore rules include askHistory `answerFromDocuments` + `refusalReason`. Phase 15 `assistantReceipts` rules are in the repo, **not deployed**. Do not deploy unless he names the project and surface. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (271.2 KB at Parts D–E). The build still fails on breach. Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
