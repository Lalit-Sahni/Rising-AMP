...
5. **Next:** Staging and production are **live** from `phase-18-people` (19 Sep 2026). Do not merge to `master`/`main`. Missing `assistantWritesEnabled` means writes are **on**; explicit `false` is off. Party backfill and job-file re-extract still refuse `--production`. Metro Consulting and the cross-kind unlinked list remain the owner's. Localhost stays on staging. The model never calculates. GST is the stated figure only. Functions are the original eight with `sendJobInviteEmail` updated; **`resendWebhook` is not deployed** (needs the real Resend signing secret at a masked prompt). Deploy by name. **No `--force`.** **400 KB is the held ceiling** (271.0 KB). The build still fails on breach. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`. On the phone: force-close and reopen the home-screen app twice.
...
- `PHASE18.md` — People (roles, page, identity bugs; staging and production live 19 Sep 2026 from `phase-18-people`).
...
- `PHASE15.md` — Ask does things (UI + `assistantReceipts` rules live with that ship; missing `assistantWritesEnabled` now means on).
