...
5. **Next:** Phase 13 is done on `phase-13-query-layer`. Do not start Phase 14 (or any new brief) until the owner names it. Production is untouched this phase. Production functions stay `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile` and `maintainLedgerRollup`. Staging also has `extractJobFileText`. Do not deploy unless he names the project and surface. Do not use `--force` on a functions deploy. **275 KB is the held ceiling** (270.1 KB). Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`.
