...
1. Confirm git branch. Latest work is **`phase-18-people`** (not merged to `master`/`main`). Restore tag: `pre-phase18-2026-09-18`. Safety tags: `cleanup-base-phase17`, `cleanup-base-phase18`, `cleanup-base-ui`, `cleanup-base-hotfix`. **Staging is live** from this trunk (19 Sep 2026): Firestore rules, Storage rules, `sendJobInviteEmail`, hosting `index-g3uSRedv.js`. **Production is not.** Production hosting currently serves `index-D_v50ECA.js`. Older restore tags: `pre-phase17-2026-09-14`, `pre-phase16-2026-09-09`, `pre-phase15-2026-09-08`, `pre-phase14-2026-09-07`, `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`. Never commit to `master` or `main`.
2. Read `PROGRESS.md` (next concrete step), then `PHASE18.md`, then `PHASE17.md`, then `PHASE17-CLOSEOUT.md`. Open `design/risingamp-people-vision.html` before writing People code; `design/risingamp-ask-vision.html` before writing Ask code; `design/risingamp-vision.html` before changing the shell. `PHASE16.md` through `PLAN.md` are closed records. `ARCHITECTURE.md` is how the running app is built. `DATABASE.md` is the living database guide.
...
5. **Next:** Staging is **live** from `phase-18-people` (19 Sep 2026). Do not merge to `master`/`main`. Do not walk production until he names it. On staging, missing `assistantWritesEnabled` means writes are **on**; explicit `false` is off. Production still runs the old code (missing = off). Party backfill and job-file re-extract still refuse `--production`. Metro Consulting and the cross-kind unlinked list remain the owner’s. Localhost stays on staging. The model never calculates. GST is the stated figure only. Staging functions are the original eight with `sendJobInviteEmail` updated; **`resendWebhook` is not deployed** (needs the real Resend signing secret at a masked prompt). Production functions are still `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile`, `maintainLedgerRollup`, `extractJobFileText` and `askRisingAmp`. Deploy by name. **No `--force`.** **400 KB is the held ceiling** (271.0 KB). The build still fails on breach. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
- Live shopfront: https://risingamp.com.au (same app as https://rising-amp-467702-b5.web.app). Staging shopfront: https://rising-amp-staging.web.app
...
- People vision (Phase 18): `design/risingamp-people-vision.html`
...
- `PHASE18.md` — People (roles, page, identity bugs; staging live 19 Sep 2026 from `phase-18-people`; production not walked).
- `PHASE17.md` — Make coding work (on the same trunk; Part E numbers in `PROGRESS.md`).
- `PHASE16.md` — The job knows itself (live on production 11 Sep 2026 from `phase-16-job-facts`; Parts A–D; not merged to master).
- `PHASE15.md` — Ask does things (UI + `assistantReceipts` rules live with that ship; production still missing = off).
