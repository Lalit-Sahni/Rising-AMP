...
1. Confirm git branch. **Phase 2 is live** on `phase-2-visual` (hosting deployed 2026-08-23). Phase 1 is on `phase-1-foundation`. **Phase 3 UI is on `phase-3-vision` (from `phase-2-visual`), running on localhost against staging.** Never commit to `master` or `main`. Restore tag if you need to unwind Phase 1: `pre-phase1-2026-08-22`.
...
5. **Next:** Lalit reviews localhost (staging), including the new sign-in / sign-up / profile flow. Production hosting has **not** been deployed. Do not write to production Firestore. Do not add a working “New job” create. Do not deploy Cloud Functions.
...
- Live: https://rising-amp-467702-b5.web.app (production) — still Phase 2 until a hosting deploy is asked for.
...
Google or email/password via Firebase Auth (email/password is on **staging** for localhost). After login, anyone can use the app. Family jobs still only show for emails on that job’s invite list (Firestore rules). Tracker data lives under `organizations/opal-ss-constructions/projects/{projectId}/…`. User profiles live under `profiles/{uid}` (staging rules). Old `users/{accessCode}` trees still exist as unused copies. Do not delete them unless the owner later asks.
...
## Phase 3 (current — localhost / staging)

Same app, same data. Brief: `PHASE3.md`. Mockup: `design/risingamp-vision.html`. Jobs home, job overview (verdict + what needs you), RisingAMP naming, and OCR “Check this” when a field was not actually read. **No production deploy. No New job write. No schema migration.**

## Out of scope until asked

Billing, Stripe, a second product, deleting leftover PIN folders, deploying functions, new npm packages (fonts via Google Fonts are OK), creating job lists from the app, production hosting deploy.
