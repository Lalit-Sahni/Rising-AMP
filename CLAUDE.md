...
1. Confirm git branch. Latest work is **`phase-16-job-facts`**. Restore tag: `pre-phase16-2026-09-09` (before Phase 16 code; SHA `a3fba94` on `phase-15-actions`). Phase 16 **Parts A–B are done** (facts record with provenance; propose from estimate / HIA / invoices). **Part C is next** (Details panel, Overview lead, one what-needs-you line). Phase 15 Parts A–E are done on `phase-15-actions` (not merged; not deployed). Phase 14 Parts A–G are done on `phase-14-ask`. Phase 15 **lifts** Phase 14’s “no writes” rule for reversible internal actions only — see `PHASE15.md`. The model still never calculates and never chooses the tier. GST is the stated figure only (never divide by 11). Phase 13 Parts A1–E plus blockers are **done on `phase-13-query-layer`**. Production is **untouched** (still the original six functions; no `askRisingAmp` on production). Phase 12 is **closed** and **live on production hosting** (6 Sep 2026; record `PHASE12.md`). **Phase 11 Parts A–E are live on production** (hosting, Firestore rules, `maintainLedgerRollup`, `ledgerRollup/current`) as of 5 Sep 2026, restore tag `pre-phase11-2026-09-05`. Never commit to `master` or `main`. Older restore tags: `pre-phase15-2026-09-08`, `pre-phase14-2026-09-07`, `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
...
5. **Next:** Phase 16 Part C (show job details where they belong, with their source). Parts A–B are **done** on `phase-16-job-facts`. Phase 13 blockers closed on staging: uncoded pool; Lalit + Sydney Excavation merges; re-extract. Metro Consulting and the cross-kind unlinked list remain the owner’s. Production is **untouched**. Localhost stays on staging. The model never calculates. GST is the stated figure only. Production functions stay `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile` and `maintainLedgerRollup` — **no `askRisingAmp` on production**. Staging also has `extractJobFileText` and `askRisingAmp` (`retry: false`). Staging Firestore rules include askHistory `answerFromDocuments` + `refusalReason`. Phase 15 `assistantReceipts` rules and Phase 16 `facts/current` rules are in the repo, **not deployed**. Do not deploy unless he names the project and surface. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (271.2 KB). The build still fails on breach. Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
> Read CLAUDE.md, then PROGRESS.md, then PHASE16.md, then PHASE15.md. Latest branch is `phase-16-job-facts`. Restore tag: pre-phase16-2026-09-09. Phase 16 Parts A–B are done (facts record; propose from estimate, HIA, invoices — nothing auto-writes). Part C is next. Phase 15 Parts A–E are done on phase-15-actions (not merged; not deployed). The model never calculates and never chooses the tier. GST is the stated figure only. Kill switch `assistantWritesEnabled` ships off (missing = off). Metro Consulting and the cross-kind unlinked list remain the owner’s. Production is untouched (six functions; no askRisingAmp). Localhost stays on staging. Never `--force`. 400 KB is the held ceiling; the build still fails on breach. Never hard-delete user records. Never accept a pasted API key. Do not deploy unless named. Look first: scripts/party-backfill-unlinked-staging.md.
...
- `PHASE16.md` — The job knows itself (open on `phase-16-job-facts`; Parts A–B done; Part C next).
- `PHASE15.md` — Ask does things (done on `phase-15-actions`; Parts A–E; not merged; not deployed).
- `PHASE14.md` — Ask (done on `phase-14-ask`; write ban lifted for Phase 15 reversible actions).
- `PHASE13.md` — The tidy workplace (done on the branch; not on production).
