...
1. Confirm git branch. Latest work is **`phase-14-ask`**. Restore tag: `pre-phase14-2026-09-07` (before Phase 14 code; SHA `0dfcb51`). Phase 14 **Parts A–D are done** (`askRisingAmp` router; palette answers; working line; question history). Part E is next (evals). Phase 13 Parts A1–E plus blockers are **done on `phase-13-query-layer`**. Production is **untouched** (still the original six functions; no `askRisingAmp` on production). Phase 12 is **closed** and **live on production hosting** (6 Sep 2026; record `PHASE12.md`). **Phase 11 Parts A–E are live on production** (hosting, Firestore rules, `maintainLedgerRollup`, `ledgerRollup/current`) as of 5 Sep 2026, restore tag `pre-phase11-2026-09-05`. Never commit to `master` or `main`. Older restore tags: `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
2. Read `PROGRESS.md` (next concrete step), then `PHASE14.md`, then `PHASE13.md`, then `PHASE12.md`. Open `design/risingamp-ask-vision.html` before writing Ask code; `design/risingamp-vision.html` before changing the shell. `PHASE10.md` is the closed Cost Plan record. `PHASE9.md` through `PLAN.md` are closed records. `ARCHITECTURE.md` is how the running app is built. `DATABASE.md` is the living database guide.
...
5. **Next:** Phase 14 Part E (evals and guardrails). Parts A–D are **done** on `phase-14-ask`. Phase 13 blockers closed on staging: uncoded pool; Lalit + Sydney Excavation merges; re-extract. Metro Consulting and the cross-kind unlinked list remain the owner’s. Production is **untouched**. Localhost stays on staging. The model never calculates. Production functions stay `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile` and `maintainLedgerRollup` — **no `askRisingAmp` on production**. Staging also has `extractJobFileText` and `askRisingAmp` (`retry: false`). Do not deploy unless he names the project and surface. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (270.1 KB until this build). The build still fails on breach. Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
- `PHASE14.md` — Ask (open on `phase-14-ask`; Part A done; Part B next).
