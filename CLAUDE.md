...
5. **Next:** Phase 16 Part B (propose job facts from the estimate, the contract and the invoices). Part A is **done** on `phase-16-job-facts`. Phase 13 blockers closed on staging: uncoded pool; Lalit + Sydney Excavation merges; re-extract. Metro Consulting and the cross-kind unlinked list remain the owner’s. Production is **untouched**. Localhost stays on staging. The model never calculates. GST is the stated figure only. Production functions stay `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile` and `maintainLedgerRollup` — **no `askRisingAmp` on production**. Staging also has `extractJobFileText` and `askRisingAmp` (`retry: false`). Staging Firestore rules include askHistory `answerFromDocuments` + `refusalReason`. Phase 15 `assistantReceipts` rules are in the repo, **not deployed**. Do not deploy unless he names the project and surface. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (271.2 KB). The build still fails on breach. Optional leftover: force-close the home-screen app twice so the Phase 12 worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
- `PHASE16.md` — The job knows itself (open on `phase-16-job-facts`; Part A done; Part B next).
