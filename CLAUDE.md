...
1. Confirm git branch. Latest work is **`phase-16-job-facts`** (pushed; **not merged to `master`/`main`**). Restore tag: `pre-phase16-2026-09-09` (before Phase 16 code; SHA `a3fba94` on `phase-15-actions`). Phase 16 **Parts A–D are done** and **live on production** 11 Sep 2026 (hosting `index-DhMkWQ3T.js`, Firestore rules, `extractJobFileText`, `askRisingAmp`). Phase 15 Parts A–E shipped on the same hosting/rules (kill switch still **off**). Phase 14 Parts A–G are done on `phase-14-ask`. Phase 15 **lifts** Phase 14’s “no writes” rule for reversible internal actions only — see `PHASE15.md`. The model still never calculates and never chooses the tier. GST is the stated figure only (never divide by 11). Phase 13 Parts A1–E plus blockers are **done on `phase-13-query-layer`**. Production backup before this go-live: `backups/production-2026-09-11T12-03-06-245Z` (528 Firestore documents, 36 Storage files). Phase 12 is **closed** and **live on production hosting** (6 Sep 2026; record `PHASE12.md`). **Phase 11 Parts A–E are live on production** (hosting, Firestore rules, `maintainLedgerRollup`, `ledgerRollup/current`) as of 5 Sep 2026, restore tag `pre-phase11-2026-09-05`. Never commit to `master` or `main`. Older restore tags: `pre-phase15-2026-09-08`, `pre-phase14-2026-09-07`, `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
2. Read `PROGRESS.md` (next concrete step), then `PHASE16.md`, then `PHASE15.md`, then `PHASE14.md`, then `PHASE13.md`. Open `design/risingamp-ask-vision.html` before writing Ask code; `design/risingamp-vision.html` before changing the shell. `PHASE10.md` is the closed Cost Plan record. `PHASE9.md` through `PLAN.md` are closed records. `ARCHITECTURE.md` is how the running app is built. `DATABASE.md` is the living database guide.
...
5. **Next:** Phase 16 is **live on production hosting, Firestore rules, `extractJobFileText` and `askRisingAmp`** (11 Sep 2026). Do not merge to `master`/`main`. Do not redeploy `maintainLedgerRollup` or recompute production rollups unless named (script still refuses `--production`; production rollups stay the Phase 11 shape). Kill switch `assistantWritesEnabled` stays **off** (missing = off); do not turn it on unless he names it. Phase 13 blockers closed on staging: uncoded pool; Lalit + Sydney Excavation merges; re-extract. Metro Consulting and the cross-kind unlinked list remain the owner’s. Localhost stays on staging. The model never calculates. GST is the stated figure only. Production functions are `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile`, `maintainLedgerRollup`, **`extractJobFileText` and `askRisingAmp`** (`retry: false` on those two). Staging has the same eight. Staging Firestore rules match the repo (`facts/current`, `assistantReceipts`, askHistory `jobFacts`). Storage rules were not redeployed. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (271.2 KB). The build still fails on breach. **Phone:** force-close and reopen the home-screen app twice so the new service worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
- Ask vision (Phase 14): `design/risingamp-ask-vision.html`
...
- Never run `firebase deploy` against production unless the owner explicitly asks. Hosting only when named: `firebase deploy --project production --only hosting`. Do not deploy functions, Firestore rules, or Storage unless he names them. Production functions are `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile`, `maintainLedgerRollup`, `extractJobFileText` and `askRisingAmp`. Deploy functions **by name**. **No `--force`.** Do not redeploy `maintainLedgerRollup` unless he also names a production recompute.
...
## Phase 12 (live on production hosting, 6 Sep 2026)

Brief and record: `PHASE12.md`. Restore tag: `pre-phase12-2026-09-05`. Front-end only: no rules, functions, schema or data changes. Dead code removed (`jobSummaries.js`, `firebaseService.js`, `invoiceNumber.ts`, `premium-animations.css`, unused CSS, unused exports, `recharts`). Real toasts. Real search palette. Sign out moved to the sidebar footer and Profile. Phone tab bar inside a job. Budget tracking retired (`/budget` redirects to Cost plan). Clients moved under the job. Add expense without fake buttons; expense form and scanner on the design tokens; Scan a receipt is a white card, not a dark slab. History and Invoices have phone card lists, Australian dates and real money formatting. One `InvoiceDocument` with the builder's profile details drives the preview, the PDF and HIA progress claims. HIA contracts no longer 'read' a fake contract; stages are typed in. Jobs home shows cost to date from the rollup. Initial JS gzip **267.9 KB** (ceiling 275). **Production hosting deployed 6 Sep 2026.** Force-close and reopen the home-screen app twice.
...
- `PHASE16.md` — The job knows itself (live on production 11 Sep 2026 from `phase-16-job-facts`; Parts A–D; not merged to master).
- `PHASE15.md` — Ask does things (UI + `assistantReceipts` rules live with that ship; kill switch still off).
- `PHASE14.md` — Ask (done on `phase-14-ask`; write ban lifted for Phase 15 reversible actions).
- `PHASE13.md` — The tidy workplace (`extractJobFileText` live on production 11 Sep 2026; production rollups still Phase 11 shape).
