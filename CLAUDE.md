...
1. Confirm git branch. Latest work is **`phase-9-job-files`**. Phase 9 **Part D** is done (Files screen: search, type counts, list/grid, viewer, receipts read-only). Staging Firestore + Storage rules are live so localhost Files works. Production is not. Phases 1–8 are live. Localhost still uses **staging**. Never commit to `master` or `main`. Restore tags: `pre-phase9-2026-08-31` (this phase), `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
...
5. **Next:** Phase 9 **Part E** — file gaps in What needs you today, and linking files to expenses/invoices. One part per session. Do not deploy unless he names it. Leftovers still parked: App Check **enforcement**, ledger rollups, normalising stored money fields, TanStack Query on the ledger, dismantling the remaining AppContext ledger blob, `PHASE6-INTEGRITY.md`.
...
Brief: `PHASE9.md`. Mockup: `design/risingamp-files-vision.html`. Branch: `phase-9-job-files`. Restore tag: `pre-phase9-2026-08-31`. **One part per session.** Parts A–D are done. Next is Part E: file gaps in What needs you today, and linking files to records. No folders. No video. Do not deploy unless named.
