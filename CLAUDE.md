...
1. Confirm git branch. Latest work is **`phase-16-job-facts`** (pushed; **not merged to `master`/`main`**). Restore tag: `pre-phase16-2026-09-09` (before Phase 16 code; SHA `a3fba94` on `phase-15-actions`). Phase 16 **Parts A–D are done**. Production was walked **12 → 13 → 14 → 15 → 16** on 11 Sep 2026 (hosting `index-DhMkWQ3T.js`, Firestore rules, `extractJobFileText`, `askRisingAmp`, `maintainLedgerRollup` with byTrade/byParty, job+org rollups recomputed). Phase 15 Parts A–E shipped on that ladder (kill switch still **off**). Phase 14 Parts A–G are done on `phase-14-ask`. Phase 15 **lifts** Phase 14’s “no writes” rule for reversible internal actions only — see `PHASE15.md`. The model still never calculates and never chooses the tier. GST is the stated figure only (never divide by 11). Phase 13 Parts A1–E plus blockers are **done on `phase-13-query-layer`**. Production backup before this go-live: `backups/production-2026-09-11T12-03-06-245Z` (528 Firestore documents, 36 Storage files). Phase 12 is **closed**. **Phase 11 Parts A–E remain the rollup trigger origin**; the function was updated 11 Sep 2026. Never commit to `master` or `main`. Older restore tags: `pre-phase15-2026-09-08`, `pre-phase14-2026-09-07`, `pre-phase13-2026-09-06`, `pre-phase12-2026-09-05`, `pre-phase10-2026-09-02`, `pre-phase10-2026-08-31`, `pre-phase9-2026-08-31`, `pre-phase8-2026-08-28`, `pre-phase7-2026-08-28`, `pre-phase6-2026-08-27`, `pre-phase1-2026-08-22`.
...
5. **Next:** Phase 16 is **live** after a **12 → 13 → 14 → 15 → 16** production walk (11 Sep 2026). Do not merge to `master`/`main`. Production rollups are the Phase 13 shape (byTrade/byParty + org rollup; 72 Centenary Dr `costCents=79758713` / 131 live; Kelly St `costCents=1198372` investor `5574194` / 8 live; second dry-run `0 write(s)`). Kill switch `assistantWritesEnabled` stays **off** (missing = off); do not turn it on unless he names it. Party backfill and job-file re-extract still refuse `--production` (forward party writes create ids; new uploads extract). Metro Consulting and the cross-kind unlinked list remain the owner’s. Localhost stays on staging. The model never calculates. GST is the stated figure only. Production functions are `sendJobInviteEmail`, `readReceiptImage`, `allocateInvoiceNumber`, `checkEstimateImport`, `readQuoteFile`, `maintainLedgerRollup`, **`extractJobFileText` and `askRisingAmp`**. Staging has the same eight. Storage rules were not redeployed. Do not use `--force` on a functions deploy. **400 KB is the held ceiling** (271.2 KB). The build still fails on breach. **Phone:** force-close and reopen the home-screen app twice so the new service worker is in. Owner list: `scripts/party-backfill-unlinked-staging.md`. ADR: `docs/adr-ask-model.md`.
...
- `PHASE13.md` — The tidy workplace (`extractJobFileText` and Phase 13 rollups live on production 11 Sep 2026).
