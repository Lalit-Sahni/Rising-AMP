 ...
 */
export async function renderInvoicePdf({ invoice, business, jobName, siteAddress, title, gstNote }: Options) {
  ...
  try {
    root.render(<InvoiceDocument invoice={invoice} business={business} jobName={jobName} siteAddress={siteAddress} title={title} gstNote={gstNote} fixedWidth />);
    await nextFrame();
    await nextFrame();
