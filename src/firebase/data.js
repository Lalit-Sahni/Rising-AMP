...
// Site Log functions
export const addSiteLogToFirestore = async (accessCode, logData) => {
  try {
    const userDocRef = await ensureUserDocument(accessCode);
    const siteLogsCollectionRef = collection(userDocRef, 'siteLogs');
    
    // Convert date string to Date object for proper Firestore storage
    const dateToStore = logData.date ? new Date(logData.date) : new Date();
    
    const docRef = await addDoc(siteLogsCollectionRef, {
      ...logData,
      date: dateToStore,
      timestamp: serverTimestamp(),
      createdAt: serverTimestamp()
    ...
    return { success: true, siteLog: newLog };
  } catch (error) {
...
export const fetchSiteLogsFromFirestore = async (accessCode) => {
  try {
    const userDocRef = await ensureUserDocument(accessCode);
    const siteLogsCollectionRef = collection(userDocRef, 'siteLogs');
    const siteLogsQuery = query(
      siteLogsCollectionRef,
      orderBy('date', 'desc')
    );
    const siteLogsSnapshot = await getDocs(siteLogsQuery);
...
export const updateSiteLogInFirestore = async (accessCode, logId, updates) => {
  try {
    const userDocRef = await ensureUserDocument(accessCode);
    ...
    if (updatesToStore.date) {
      updatesToStore.date = new Date(updatesToStore.date);
    }
    ...
    const updatedLog = {
      id: logId,
      ...updates,
      date: updatesToStore.date || updates.date,
      updatedAt: new Date()
    };

    return { success: true, siteLog: updatedLog };
  } catch (error) {
...
export const deleteSiteLogFromFirestore = async (accessCode, logId) => {
  try {
    const userDocRef = await ensureUserDocument(accessCode);
    const logDocRef = doc(userDocRef, 'siteLogs', logId);
