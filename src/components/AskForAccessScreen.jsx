...
export default function AskForAccessScreen({ email, onSignOut, reason, onRetry }) {
  const lookupFailed = reason === 'lookup-failed';
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-surface border border-hairline rounded-ot p-8 shadow-whisper text-center">
        ...
        {lookupFailed ? (
          <>
            <h1 className="text-[20px] font-extrabold tracking-tight">Could not check access</h1>
            <p className="text-[13.5px] text-slate-600 mt-3">
              We could not check whether you are on a company{email ? ` (${email})` : ''}.
              This is not a refusal. Check your connection and try again.
            </p>
          </>
        ) : (
          <>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-2 justify-center">
          ...
          >
            {lookupFailed ? 'Retry' : 'Check again'}
          </button>
          <button
            type="button"
            onClick={onSignOut}
