...
organizations/{orgId}
  ...
  assistantWritesEnabled                   # Phase 17 Part A inverted the default. Owner-only boolean. Explicit false = assistant writes off. Missing means ON on staging/localhost (code from phase-18-people, live 19 Sep 2026). Production still runs the Phase 16 client, where missing or not true = off. Undo of an already-applied receipt still works when off.
