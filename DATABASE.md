...
organizations/{orgId}
  ...
  askHistory/{uid}/items/{id}                  # Phase 14 Part D. Per-user Ask questions for this org. Job-scoped when jobId is set (empty string = all jobs). Stores the question, routed query+params, query provenance, and code-computed snapshot cents/counts. Never model sentence/reason. Owner uid may read/write/delete own rows; another member cannot. Not a chat log.
