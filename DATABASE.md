...
organizations/{orgId}
  ...
  assistantReceipts/{id}                       # Phase 15 Part A. Receipt for an assistant write: action, tier (do|propose|refuse), status (applied|proposed|refused|undone), evidence per field, clientKey, document ids, undo payload. source is always 'assistant'. Members of the org read; create with that shape; update is undo only (status undone, undoneAt, undoReceiptId). Delete denied. Not a chat log (no messages/sentence). Schema in the repo this part; not deployed to staging or production.
  ...
  projects/{jobId}                             # THE job record
    ...
    expenses/{id}          + jobId, optional tradeId (or not-in-estimate | investor). Optional source: 'assistant' and assistantReceiptId when the action layer wrote the row (Phase 15 Part A; schema in repo, not deployed). Human Cost Plan edits do not stamp those fields.
...
12. **The action layer is receipted and undoable (Phase 15 Part A, not on production).** `src/actions/` writes through membership scope, never from `src/queries/`. The one primitive is `codeExpense` (one existing expense `tradeId`, including clear). Every write returns a receipt at `organizations/{orgId}/assistantReceipts/{id}` with evidence, tier, `clientKey` idempotency, and an undo payload. Undo restores the previous `tradeId` (including null) and drops `source` / `assistantReceiptId`. It never `deleteDoc`s a live expense. The model does not calculate and does not choose the tier. Inferred evidence never reaches do. NEVER actions (email, invoice numbers, invite, archive, delete, settings, spend) refuse with no write. Schema is in the repo; nothing is deployed this part.
