# Phase 6 leftovers — Integrity and follow-through

**Parked.** Current Phase 6 is the legacy cut on branch `phase-6-legacy-cut`. Read `PHASE6.md` instead. Come back to this file after that branch is done.

Read `CLAUDE.md` then `PROGRESS.md` then this file before touching anything. Open `design/risingamp-vision.html` in a browser.

Original branch: **`phase-6-integrity`**, created from `phase-5-jobs-members` (2026-08-27). Phases 1–5 are live. Shopfront: `https://risingamp.com.au`. Localhost still uses **staging**. Never commit to `master` or `main`.
