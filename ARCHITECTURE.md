...
- `facts/current` — members can read and write a valid Phase 16 facts document (`schemaVersion: 1`); delete is denied. Rules live on staging and production (11 Sep 2026).
...
- These Phase 10 rules are live on staging and production (2 Sep 2026). Phase 16 `facts/current` and Phase 15 `assistantReceipts` rules shipped 11 Sep 2026.
...
`costPlan/current` is optional. Level 1 stores one GST-inclusive target. Level 2 adds `sections` keyed by trade. Level 3 adds imported lines under those trades. `facts/current` is optional too (`schemaVersion: 1` in `src/domain/jobFacts.ts`): every field is optional with provenance; an empty document is valid; nothing invents 0 sqm or $0. Rules live on staging and production (11 Sep 2026). Phase 16 Part B proposes facts from the BOQ cover, live HIA rows, unique client/invoice addresses, labelled permit/certificate/contract extracts, and a job name that looks like a street. Nothing auto-writes: Cost Plan and HIA lazy-load a review sheet; Accept calls `saveJobFacts`. Phase 16 Part C shows the record on Overview: a lead (address / floor area / contract value when present) and a lazy Details panel; one what-needs-you line when stored facts are unconfirmed; handover cover, invoice Job line and expense-export subtitle use present facts only. Phase 16 Part D adds a read-only Ask query `jobFacts` (membership-scoped; `facts` provenance; never calculates; never writes). The adapter is not on App.js / PaletteHost / Header / first paint. Floor area from an already imported plan cannot be recovered (re-import the spreadsheet). `quotes/{quoteId}` are separate documents with optional `fileIds` (and leftover `fileId`) pointing at `files/{fileId}`. Expenses may carry optional `tradeId` and a retaggable `category`. `ledgerRollup/current` is the server-owned expense summary (cost, counts, category and calendar buckets). Members can read it; only Cloud Functions write it. A job with no document has no Cost Plan nav item. Plan, quotes and the org trade list are shared through TanStack Query; they are not added to AppContext.
...
`askRisingAmp` (staging and production since 11 Sep 2026) uses `gpt-4o-mini`. See `docs/adr-ask-model.md`. The function returns a validated route (`query` + `params`, or `none`). It never reads the ledger and never calculates. The phone then runs `src/queries/` on the caller’s own credentials. Figures in the UI are `formatCents` on those query cents, not model prose. Phase 16 Part D adds `jobFacts` to the allow-list; the live function classifies floor-area questions as `jobFacts` after the 11 Sep 2026 deploy.

**Tokens per question (placeholder — first production traffic 11 Sep 2026).** Re-measure after real questions on the shopfront. From the request body we actually send:
...
**Phone latency, Enter to painted answer (placeholder — not timed on a production phone).** Honest expectation from Sydney to `us-central1` plus OpenAI, then one query on the device: about **1.5–4 s** on a good connection. The Iowa hop is the same ~200 ms already documented for other callables. Re-measure on a phone after the 11 Sep 2026 production deploy; do not treat these ranges as a production SLA.

---

## 17. Action layer (Phase 15; staging writes on by default from Phase 17)

Phase 15 adds a client action layer in `src/actions/`, beside the read-only query layer. The model still does not calculate. Tiers (`do` / `propose` / `refuse`) are decided in code from evidence; an inferred field never reaches do. Writes are receipted at `organizations/{orgId}/assistantReceipts/{id}` and undoable (restore the previous `tradeId`, restore a batch of them, or void a created expense — never hard-delete). Part A coded an existing expense. Part B creates an uncoded expense from a scanned receipt when supplier, amount and date are clean and a party matches exactly. Part C proposes trades for uncoded expenses from party history, then section name, then trade category; the Cost Plan sheet shows the set before writing; Accept all is confident (party history) only. Part D is the activity view (lazy `/assistant-activity`): newest-first receipts with Undo from the stored id after reload; a quiet History “Check” until a human opens or saves; a daily line in what needs you from yesterday’s receipts. GST is stored only when the invoice stated it (`gstCents`); it is never derived by dividing by 11. Palette does not execute writes. Not imported from `App.js` or `PaletteHost`. Part E adds tier, refusal, injection and undo evals, plus org `assistantWritesEnabled` (owner-only toggle on Profile; undo still works when off). Phase 17 inverted the missing case: on staging/localhost missing means on; explicit false is off. Production still runs the Phase 16 client, where missing or not true is off. Hosting and `assistantReceipts` rules live 11 Sep 2026.
