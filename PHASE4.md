...
- If you judge the **Firebase Trigger Email extension** simpler for this stack than a hand-written Cloud Function, propose that instead before building, rather than switching silently. Either way this requires deploying *something* server-side — flag that clearly to the owner first, since standing instruction elsewhere in this repo is "do not deploy Cloud Functions unless asked." This phase is that ask, scoped narrowly to invite email only. Do not touch or redeploy the existing unused `generateWeeklyReport` function.
